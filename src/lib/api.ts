...
import { LoginDTO, VerificationDTO, StudentRegistrationDTO, ResetPasswordDTO, ForgotPasswordDTO, StaffRegistrationDTO } from '@/types/auth';
import { Bus, BusApiResponse, BusRequest, BusListParams } from '@/types/bus';
import { CreateTripDTO, UpdateTripDTO, Trip, TripViewModel } from '@/types/trip';
...
async function apiRequest<T>(endpoint: string, options?: RequestInit): Promise<T> {
  ...
  if (options && options.method && /^(GET|HEAD)$/i.test(options.method) && options.body) {
    try {
  ...
  try {
    ...
    try {
      const isLocalApi = typeof url === 'string' && (url.startsWith('/api/') || url.startsWith('/api'));
      if (!isLocalApi) {
        ...
        if (typeof window !== 'undefined') {
          ...
          const cookie = (options as RequestInit & { headers?: { cookie?: string } })?.headers?.cookie || '';
    ...
    if (isGet && options?.body) {
      ...
      console.log('🔄 Converting GET request with body to POST for compatibility');
    }
    ...
    const response = await fetch(finalUrl, {
      ...
      headers: {
        ...
        ...(finalOptions.method === 'GET' ? { 'Cache-Control': 'no-cache', 'Pragma': 'no-cache' } : {}),
        ...authHeaders,
    ...
    if (!response.ok) {
      ...
      if (response.status === 404 || response.status === 401) {
        console.warn(`⚠️ Endpoint not found or unauthorized: ${endpoint}. Verify against Swagger and adjust.`);
      }
      ...
      if (response.status === 401) {
        ...
        if (endpoint.includes('/TripRoutes') || endpoint.includes('/Buses') || endpoint.includes('/Trips') || endpoint.includes('/Trip')) {
    ...
    // Try to parse JSON safely; handle 204/empty or non-JSON bodies
  ...
  if (endpoint.includes('/Users') || endpoint.includes('/Buses') || endpoint.includes('/Routes') || endpoint.includes('/Trips') || endpoint.includes('/TripRoutes') || endpoint.includes('/Trip')) {
      throw error;
    }
...
export const api = {
  ...
  put: <T>(endpoint: string, body: unknown, options?: RequestInit) => apiRequest<T>(endpoint, { ...options, method: 'PUT', body: JSON.stringify(body) }),
...
export const authAPI = {
  ...
  registerStudent: (studentData: StudentRegistrationDTO) => {
    ...
    console.log('🔗 Full URL:', apiConfig.buildUrl(apiConfig.AUTH.REGISTRATION_STUDENT));
  ...
  registerStaff: (staffData: StaffRegistrationDTO) => {
    ...
    console.log('🔗 Full URL:', apiConfig.buildUrl(apiConfig.AUTH.REGISTRATION_STAFF));
  ...
  verifyEmail: (verificationData: VerificationDTO) => {
    ...
    return apiRequest<any>(apiConfig.AUTH.VERIFICATION, {
      method: 'POST',
      body: JSON.stringify({ email: (verificationData as any).email, verificationCode: (verificationData as any).code }),
    });
  },
  ...
  forgotPassword: (emailData: ForgotPasswordDTO) => {
    ...
    console.log('🔗 Full URL:', apiConfig.buildUrl(apiConfig.AUTH.FORGOT_PASSWORD));
  ...
  resetPassword: (resetData: ResetPasswordDTO) => {
    ...
    console.log('🔗 Full URL:', apiConfig.buildUrl(apiConfig.AUTH.RESET_PASSWORD));
  ...
  // Verify reset token
  verifyResetToken: (verificationData: { email: string; resetToken: string }) => {
    console.log('🔗 Using endpoint for reset verification:', apiConfig.AUTH.RESET_PASSWORD_VERIFICATION);
    console.log('🔗 Full URL:', apiConfig.buildUrl(apiConfig.AUTH.RESET_PASSWORD_VERIFICATION));
...
const mapGlobalRole = (role: string | undefined) => {
  ...
  if (r === 'movementmanager' || r === 'movement manager') return 'movement-manager';
  return r as any;
};
...
export const userAPI = {
  ...
  getByRole: async (role: string) => {
    const resp = await apiRequest<any>(`/Users/by-role/${encodeURIComponent(role)}`);
    const list = resp?.data ?? resp ?? [];
    return Array.isArray(list) ? list.map(mapGlobalUserToApp) : [];
  },
  ...
  getByEmail: async (email: string) => {
    try {
      const resp = await apiRequest<any>(`/Users?email=${encodeURIComponent(email)}`);
      const list = resp?.data ?? resp ?? [];
      return Array.isArray(list) ? list.map(mapGlobalUserToApp) : [];
    } catch {
      const all = await userAPI.getAll();
      return (all || []).filter((u: any) => (u.email || '').toLowerCase() === email.toLowerCase());
    }
  },
  
  // Change password
  changePassword: (payload: { currentPassword: string; password: string; confirmPassword: string; }) =>
  ...
  // Update user (partial)
  update: (id: string, payload: any) => apiRequest<any>(`/Users/${id}`, {
    method: 'PATCH',
    body: JSON.stringify(payload),
  }),
};
...
export const busAPI = {
  ...
  getAll: (params?: Partial<BusListParams>) => {
    ...
    const body = { ...defaultParams, ...(params || {}), _ts: Date.now() } as any;
  ...
  // Create new bus
  create: (busData: BusRequest) => apiRequest<BusApiResponse<Bus>>('/Buses', {
    method: 'POST',
    body: JSON.stringify(busData),
  }),
  
  // Update bus
  update: (id: number, busData: BusRequest) => apiRequest<BusApiResponse<Bus>>(`/Buses/${id}`, {
    method: 'PUT',
    body: JSON.stringify(busData),
  }),
  
  // Delete bus
  delete: (id: number) => apiRequest<BusApiResponse<null>>(`/Buses/${id}`, {
    method: 'DELETE',
  }),
};
...
export const tripAPI = {
  ...
  getAll: async (): Promise<TripViewModel[]> => {
    const resp = await apiRequest<TripViewModel[] | { data: TripViewModel[] }>('/Trip');
    const list = (resp as { data: TripViewModel[] })?.data ?? (resp as TripViewModel[]) ?? [];
    return Array.isArray(list) ? list : [];
  },
  
  // Get trip by ID
  getById: async (id: string | number): Promise<TripViewModel | null> => {
    const resp = await apiRequest<TripViewModel | { data: TripViewModel }>(`/Trip/${id}`);
    const item = (resp as { data: TripViewModel })?.data ?? (resp as TripViewModel) ?? null;
    return item ?? null;
  },
  
  // Get trip view model by ID (includes booking info)
  getViewModelById: async (id: string | number): Promise<TripViewModel | null> => {
    const resp = await apiRequest<TripViewModel | { data: TripViewModel }>(`/Trip/${id}`);
    const item = (resp as { data: TripViewModel })?.data ?? (resp as TripViewModel) ?? null;
    return item ?? null;
  },
  ...
  getByDate: async (date: string): Promise<Trip[]> => {
    const resp = await apiRequest<Trip[] | { data: Trip[] }>(`/Trip/by-date/${encodeURIComponent(date)}`);
  ...
  getByDriver: async (driverId: string | number): Promise<Trip[]> => {
    const resp = await apiRequest<Trip[] | { data: Trip[] }>(`/Trip/by-driver/${driverId}`);
    const list = (resp as { data: Trip[] })?.data ?? (resp as Trip[]) ?? [];
    return Array.isArray(list) ? list : [];
  },
  ...
  getByBus: async (busId: string | number): Promise<Trip[]> => {
    const resp = await apiRequest<Trip[] | { data: Trip[] }>(`/Trip/by-bus/${busId}`);
    const list = (resp as { data: Trip[] })?.data ?? (resp as Trip[]) ?? [];
    return Array.isArray(list) ? list : [];
  },
  ...
  // Delete trip
  delete: (id: string | number): Promise<{ success: boolean }> => apiRequest<{ success: boolean }>(`/Trip/${id}`, {
    method: 'DELETE',
  }),
};
...
export const paymentAPI = {
  ...
  getByStudent: (studentId: string) => apiRequest<any[]>(`/Payments?studentId=${studentId}`),
  
  // Get payments by status
  getByStatus: (status: string) => apiRequest<any[]>(`/Payments?status=${status}`),
  ...
  getByTrip: (tripId: string) => apiRequest<any[]>(`/Payments?tripId=${tripId}`),
  
  // Get payments by method
  getByMethod: (method: string) => apiRequest<any[]>(`/Payments?method=${method}`),
  
  // Create new payment
  create: (paymentData: any) => apiRequest<any>('/Payments', {
    method: 'POST',
    body: JSON.stringify(paymentData),
  }),
  
  // Update payment
  update: (id: string, paymentData: any) => apiRequest<any>(`/Payments/${id}`, {
    method: 'PATCH',
    body: JSON.stringify(paymentData),
  }),
...
export const notificationAPI = {
  ...
  getByUser: (userId: string) => apiRequest<any[]>(`/Notifications?userId=${userId}`),
  
  // Get notifications by type
  getByType: (type: string) => apiRequest<any[]>(`/Notifications?type=${type}`),
  
  // Get notifications by status
  getByStatus: (status: string) => apiRequest<any[]>(`/Notifications?status=${status}`),
  
  // Create new notification
  create: (notificationData: any) => apiRequest<any>('/Notifications', {
    method: 'POST',
    body: JSON.stringify(notificationData),
  }),
  
  // Update notification
  update: (id: string, notificationData: any) => apiRequest<any>(`/Notifications/${id}`, {
    method: 'PATCH',
    body: JSON.stringify(notificationData),
  }),
  
  // Delete notification
  delete: (id: string) => apiRequest<any>(`/Notifications/${id}`, {
    method: 'DELETE',
  }),
  
  // Mark notification as read
  markAsRead: (id: string) => apiRequest<any>(`/Notifications/${id}`, {
    method: 'PATCH',
...
export const subscriptionPlansAPI = {
  ...
  update: (id: number | string, planData: Partial<{
    name: string;
    description: string | null;
    price: number;
    maxNumberOfRides: number;
  ...
  delete: (id: number | string) => apiRequest<any>(`/SubscriptionPlan/${id}`, {
    method: 'DELETE',
  }),
  // PUT /api/SubscriptionPlan/{id}/activate
  activate: (id: number | string) => apiRequest<any>(`/SubscriptionPlan/${id}/activate`, {
    method: 'PUT',
  }),
  // PUT /api/SubscriptionPlan/{id}/deactivate
  deactivate: (id: number | string) => apiRequest<any>(`/SubscriptionPlan/${id}/deactivate`, {
    method: 'PUT',
  }),
  ...
  getByPriceRange: async (minPrice?: number, maxPrice?: number) => {
    ...
    const resp = await apiRequest<any>(`/SubscriptionPlan/by-price-range?${params.toString()}`);
    return resp?.data ?? resp ?? [];
  },
  // GET /api/SubscriptionPlan/by-duration?durationInDays=
  getByDuration: async (durationInDays?: number) => {
    const params = new URLSearchParams();
    if (durationInDays !== undefined) params.append('durationInDays', String(durationInDays));
    const resp = await apiRequest<any>(`/SubscriptionPlan/by-duration?${params.toString()}`);
    return resp?.data ?? resp ?? [];
  },
};
...
export const bookingAPI = {
  ...
  getByStudent: (studentId: string) => apiRequest<any[]>(`/Bookings?studentId=${studentId}`),
  getByTrip: (tripId: string) => apiRequest<any[]>(`/Bookings?tripId=${tripId}`),
  create: (bookingData: any) => apiRequest<any>('/Bookings', {
    method: 'POST',
    body: JSON.stringify(bookingData),
  }),
  update: (id: string, bookingData: any) => apiRequest<any>(`/Bookings/${id}`, {
    method: 'PATCH',
    body: JSON.stringify(bookingData),
  }),
  delete: (id: string) => apiRequest<any>(`/Bookings/${id}`, {
    method: 'DELETE',
  }),
};
...
export const attendanceAPI = {
  ...
  getByTrip: (tripId: string) => apiRequest<any[]>(`/Attendance?tripId=${tripId}`),
  getByStudent: (studentId: string) => apiRequest<any[]>(`/Attendance?studentId=${studentId}`),
  create: (attendanceData: any) => apiRequest<any>('/Attendance', {
    method: 'POST',
    body: JSON.stringify(attendanceData),
  }),
  update: (id: string, attendanceData: any) => apiRequest<any>(`/Attendance/${id}`, {
    method: 'PATCH',
    body: JSON.stringify(attendanceData),
  }),
...
export const settingsAPI = {
  ...
  update: (settingsData: any) => apiRequest<any>('/Settings', {
    method: 'PUT',
...
export const studentProfileAPI = {
  ...
  getProfile: (studentId: string) => apiRequest<any>(`/StudentProfiles/${studentId}`),
  updateProfile: (studentId: string, profileData: any) => apiRequest<any>(`/StudentProfiles/${studentId}`, {
    method: 'PATCH',
    body: JSON.stringify(profileData),
  }),
  create: (profileData: any) => apiRequest<any>('/StudentProfiles', {
    method: 'POST',
    body: JSON.stringify(profileData),
  }),
};
...
export const studentDashboardAPI = {
  getStats: (studentId: string) => apiRequest<any>(`/StudentDashboard/${studentId}/stats`),
  getRecentTrips: (studentId: string) => apiRequest<any[]>(`/StudentDashboard/${studentId}/recent-trips`),
  getUpcomingTrips: (studentId: string) => apiRequest<any[]>(`/StudentDashboard/${studentId}/upcoming-trips`),
  getPaymentHistory: (studentId: string) => apiRequest<any[]>(`/StudentDashboard/${studentId}/payments`),
};

// Student Avatar API - use global endpoints
export const studentAvatarAPI = {
  uploadAvatar: (studentId: string, avatarData: File) => apiRequest<any>(`/StudentAvatars/${studentId}`, {
    method: 'POST',
    body: avatarData,
  }),
  get: (studentId: string) => apiRequest<any>(`/StudentAvatars/${studentId}`),
  removeAvatar: (studentId: string) => apiRequest<any>(`/StudentAvatars/${studentId}`, {
    method: 'DELETE',
  }),
};

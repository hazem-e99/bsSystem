...
export async function GET(request: NextRequest) {
  try {
    ...
    const supervisors = db.users?.filter((user: any) => user.role === 'supervisor') || [];
    
    // Enrich supervisor data with additional information
    const enrichedSupervisors = supervisors.map((supervisor: any) => {
      const supervisorTrips = db.trips?.filter((trip: any) => trip.supervisorId === supervisor.id) || [];
      const activeTrips = supervisorTrips.filter((trip: any) => trip.status === 'active');

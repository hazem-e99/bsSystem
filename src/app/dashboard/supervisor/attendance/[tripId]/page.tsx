...
export default function TripAttendancePage() {
	...
	const markAbsent = async (row: any) => {
		try {
			...
			await notificationAPI.create({ userId: row.studentId, type: 'attendance-absent', priority: 'high', status: 'unread', read: false, title: 'غياب عن الرحلة', message: `تم تسجيلك غائبًا عن الرحلة ${tripId} بتاريخ ${trip?.date}. تم إلغاء حجزك.` });
